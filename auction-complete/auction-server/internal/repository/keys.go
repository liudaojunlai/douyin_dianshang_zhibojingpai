...
// AuctionCommentsKey 房间历史评论
// List，最多存储 100 条最新评论
func AuctionCommentsKey(auctionID uint) string {
	return fmt.Sprintf("auction:%d:comments", auctionID)
}

// UserCommentRateKey 用户评论频率限制
// String，INCR，TTL：60 秒
func UserCommentRateKey(userID uint) string {
	return fmt.Sprintf("user:%d:comment_rate", userID)
}

// UserInfoCacheKey 用户信息缓存
// Hash，TTL：2 小时
func UserInfoCacheKey(userID uint) string {
